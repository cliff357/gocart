...
describe('Firestore Security Rules 測試', () => {
    ...
    describe('Carts Collection', () => {
        it('未登入用戶不能讀取購物車', async () => {
            await setAdminData('carts/user-1', {
                items: [{ productId: 'prod-1', quantity: 2 }],
            });

            const db = getUnauthenticatedDb();
            await assertFails(db.collection('carts').doc('user-1').get());
        });

        it('已登入用戶不能讀取購物車', async () => {
            await setAdminData('carts/user-1', {
                items: [{ productId: 'prod-1', quantity: 2 }],
            });
    ...
    // 新 Rules:
    // - 有專門 match
    // - read: if isAuthenticated()（需要登入）
    // - write: if isAdmin()（只有 admin 可寫）
    // ============================================
    describe('Addresses Collection', () => {
        it('未登入用戶不能讀取地址', async () => {
